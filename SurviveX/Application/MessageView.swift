    ...
    VStack(alignment: .center) {
      if message.type == .info {
        ...
        VStack(alignment: message.type == .llamagenerated ? .leading : .trailing) {
          ...
          HStack {
            if message.type != .llamagenerated { Spacer() }
            if message.text.isEmpty {
              ...
              Text(message.text)
                .padding(10)
                .foregroundColor(message.type == .llamagenerated ? .primary : .white)
                .background(message.type == .llamagenerated ? Color(UIColor.secondarySystemBackground) : Color.blue)
                .cornerRadius(20)
                .contextMenu {
                  Button(action: {
